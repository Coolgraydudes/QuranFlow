...
const Detail = () => {
  ...
  const navigate = useNavigate(); // Untuk navigasi ke bookmark
  ...
  // --- FITUR PILIH QORI (Sesuai Home.jsx) ---
  const listQori = [
  ...
  // Gunakan useState supaya audio player langsung update saat qori diganti
  ...
  const handleShare = async (ayat) => {
    ...
    if (navigator.share) {
      try { 
        await navigator.share({ title: 'QuranFlow', text: shareText, url: shareUrl }); 
      } catch (err) { 
        console.log(err); 
      }
    } else {
  ...
  const handleLastRead = (ayat) => {
    ...
    alert(`Ditandai sebagai terakhir baca: ${detailSurah.namaLatin} ayat ${ayat.nomorAyat}`);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-gray-900 dark:bg-[#0f172a] dark:text-gray-100 transition-colors duration-500 font-sans">
      <div className="container mx-auto p-4 md:p-6 flex flex-col items-center">
          ...
          <div className="w-full max-w-4xl space-y-10 mb-24">
            ...
            <div className="relative overflow-hidden bg-emerald-600 dark:bg-emerald-500 text-white p-10 md:p-14 rounded-[3rem] shadow-2xl text-center">
              ...
              <div className="bg-black/10 backdrop-blur-xl p-4 rounded-[2rem] border border-white/20 inline-block w-full max-w-md">
                <p className="text-[10px] font-bold mb-2 opacity-70 uppercase tracking-widest">
                  Qori: {listQori.find(q => q.id === selectedQori)?.nama}
                </p>
                <audio key={selectedQori} controls className="w-full h-10 accent-emerald-400">
            ...
            <div className="space-y-8">
              {detailSurah.ayat.map((ayat) => {
                const isBookmarked = bookmarks.find(b => b.id === `${detailSurah.nomor}:${ayat.nomorAyat}`);
                ...
                return (
                  <div key={ayat.nomorAyat} className="bg-white dark:bg-slate-900/40 p-8 md:p-12 rounded-[3.5rem] border border-gray-100 dark:border-gray-800 shadow-sm transition-all duration-500">
                    <div className="flex justify-between items-center mb-10">
                      <span className="bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-400 w-12 h-12 flex items-center justify-center rounded-2xl font-black">
                        {ayat.nomorAyat}
                      </span>
                      <div className="flex gap-3">
                        <button onClick={() => handleShare(ayat)} title="Bagikan" className="p-3 bg-gray-50 dark:bg-gray-800 text-gray-400 hover:text-emerald-500 rounded-xl transition-colors">
                          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" /></svg>
                        </button>
                        <button onClick={() => handleLastRead(ayat)} title="Terakhir Baca" className="p-3 bg-gray-50 dark:bg-gray-800 text-gray-400 hover:text-emerald-500 rounded-xl transition-colors">
                          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                        </button>
                        <button 
                          onClick={() => dispatch(toggleBookmark({ 
                            id: `${detailSurah.nomor}:${ayat.nomorAyat}`, 
                            ar: ayat.teksArab, 
                            text: ayat.teksIndonesia, 
                            surah: detailSurah.namaLatin 
                          }))}
                          className={`p-3 rounded-xl transition-all ${isBookmarked ? 'bg-emerald-500 text-white' : 'bg-gray-50 dark:bg-gray-800 text-gray-400'}`}
                        >
                    ...
                    <div className="text-right mb-12">
                      {/* Font diganti menjadi font-amiri */}
                      <h2 className="text-4xl md:text-6xl font-amiri font-bold leading-[2.5]" dir="rtl">{ayat.teksArab}</h2>
                    </div>
                    <div className="space-y-4 border-l-4 border-emerald-500/20 pl-6">
                      <p className="text-emerald-600 dark:text-emerald-400 font-bold italic">{ayat.teksLatin}</p>
                      <p className="text-gray-600 dark:text-gray-300 text-lg font-medium">{ayat.teksIndonesia}</p>
                    </div>
                  </div>
                );
              })}
                  ...
                  {listQori.map((q) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span>{q.nama}</span>
                        {selectedQori === q.id && <div className="w-2 h-2 bg-emerald-500 rounded-full"></div>}
                      </div>
                    </button>
                  ))}
                </div>
        ...
        {open && <div onClick={() => setOpen(false)} className="fixed inset-0 bg-slate-900/40 z-40 backdrop-blur-sm transition-all" />}

        {/* TOMBOL MENU (FLOATING) */}
        <button onClick={() => setOpen(true)} className="fixed bottom-8 right-8 bg-emerald-600 dark:bg-emerald-500 text-white w-14 h-14 flex items-center justify-center rounded-2xl shadow-xl z-30 group hover:scale-110 active:scale-95 transition-all">
          <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor"><path d="M10 3h-6a1 1 0 0 0 -1 1v6a1 1 0 0 0 1 1h6a1 1 0 0 0 1 -1v-6a1 1 0 0 0 -1 -1z" /><path d="M20 3h-6a1 1 0 0 0 -1 1v6a1 1 0 0 0 1 1h6a1 1 0 0 0 1 -1v-6a1 1 0 0 0 -1 -1z" /><path d="M10 13h-6a1 1 0 0 0 -1 1v6a1 1 0 0 0 1 1h6a1 1 0 0 0 1 -1v-6a1 1 0 0 0 -1 -1z" /><path d="M17 13a4 4 0 1 1 -3.995 4.2l-.005 -.2l.005 -.2a4 4 0 0 1 3.995 -3.8z" /></svg>
        </button>
      </div>
    </div>
  );
